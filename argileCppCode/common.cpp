...
MVector getVertexTangent(MFnMesh& fnInputMesh, MItMeshVertex& meshVertIt, int indVtx) 
{	
	int oldInd;
...
MVector getVertexTangentFromFace(MFnMesh& fnInputMesh, MIntArray& connectedFaces, int indVtx) 
{
  MVector tangent;
  int oldInd;
  MVector theTangent;
  for (unsigned int i = 0; i < connectedFaces.length(); i += 3) {
    ...
    fnInputMesh.getFaceVertexTangent(theFace, indVtx, theTangent, MSpace::kWorld);
    if (i == 0) {
      tangent = theTangent;
    }
    else {
